...
import Label from "./components/Lebal";
...
function App() {
			...
			<>
				{structure.map((element) => {
					if (element.elementName === "Label") {
						return (
							<Label
								onDClickElement={onDClickElement}
								key={element.id}
								uuid={element.id}
								x={element.x}
								y={element.y}
								...
								onDragStartCurrent={onDragStartCurrent}
							/>
						);
					}
	...
	const onDrop = async (event) => {
		...
		if (currElementId > 1000) {
			await completeStructureChange(currElementX, currElementY, event).then(
				() => setCurrElementId(0),
			);
		} else {
	...
	return (
		<div
			...
			onClick={(e) => {
				if (
					e.target === document.getElementsByClassName("App__dropzone")[0] ||
					e.target === document.getElementsByClassName("App__dragzone")[0]
				) {
					setModalVisible(false);
		...
		>
			...
			<Modal
				title={`Edit ${dragZoneSelectedElement}`}
				open={modalVisible}
				onOk={saveChange}
				onCancel={() => {
					setModalVisible(false);
			...
			>
				<hr
					className="my-4"
					style={{ border: "solid 1px #000", opacity: "0.07" }}
				/>
				<span className="modal__label">Text</span>
				<br />
				<input
				...
				/>
				<br />
				<span className="modal__label">X</span>
				<br />
				<input
					className="modal__input"
					type="number"
				...
				/>
				<br />
				<span className="modal__label">Y</span>
				<br />
				<input
					className="modal__input"
					type="number"
				...
				/>
				<br />
				<span className="modal__label">Font Size</span>
				<br />
				<input
					className="modal__input"
					type="number"
				...
				/>
				<br />
				<span className="modal__label">Font Weight</span>
				<br />
				<input
					className="modal__input"
					type="number"
			...
			<div
				...
				style={{ display: updateModalVisible ? "block" : "none" }}
			>
				{" "}
				*<p className="modal__header">Edit Label</p>
				...
				<input
					className="modal__input"
					type="text"
					onChange={(e) => setCurrElementText(e.target.value)}
					value={currElementText}
				/>
				<br />
				<span className="modal__label">X</span>
				<br />
				<input
					className="modal__input"
					type="number"
					onChange={(e) => setCurrElementX(e.target.value)}
					value={currElementX}
				/>
				<br />
				<span className="modal__label">Y</span>
				...
				<input
					className="modal__input"
					type="number"
					onChange={(e) => setCurrElementY(e.target.value)}
					value={currElementY}
				/>
				<br />
				<span className="modal__label">Font Size</span>
				...
				<input
					className="modal__input"
					type="number"
					onChange={(e) => setCurrElementFontSize(e.target.value)}
					value={currElementFontSize}
				/>
				<br />
				<span className="modal__label">Font Weight</span>
				<br />
				<input
					className="modal__input"
					type="number"
					onChange={(e) => setCurrElementFontWeight(e.target.value)}
					value={currElementFontWeight}
				/>
				<br />
				<button className="modal__savebtn" onClick={updateElement}>
					Update Changes
				</button>
				<button className="modal__savebtn" onClick={deleteElement}>
					Delete
				</button>
			</div>
